      ...
      for RPCPERF_TASKS in 4000; do
        for TOPICS in 1; do
          for STORAGE in /mnt/gp3 /mnt/data-1; do             
            cmd="systemslab --systemslab-url http://localhost submit ./rpcperf-c1-s1-steps.jsonnet --param topics=$TOPICS --param partitions=$PARTITIONS --param storage=$STORAGE --param lingerms=$LINGERMS --param rpcperf_tasks=$RPCPERF_TASKS --param rpcperf_threads=$RPCPERF_THREADS"            
            $cmd
          done
        done
