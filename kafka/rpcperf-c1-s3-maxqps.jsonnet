        ...
        steps: [ 
          ...
          bash(
            |||
              ...
              KAFKA_NETWORK_THREADS=%s
              KAFKA_IO_THREADS=%s
              sed -ie "s/BROKERID/1/g" server.properties
              sed -ie "s/ZOOKEEPER_SERVER_ADDR/$ZOOKEEPER_SERVER_ADDR/g" server.properties
              sed -ie "s/KAFKA_NETWORK_THREADS/$KAFKA_NETWORK_THREADS/g" server.properties
              sed -ie "s/KAFKA_IO_THREADS/$KAFKA_IO_THREADS/g" server.properties
              SED_KAFKA_LOG=$(sed 's/[\/&]/\\&/g' <<<"$KAFKA_LOG")                          
        ...
        steps: [ 
          ...
          bash(
            |||
              ...
              KAFKA_NETWORK_THREADS=%s
              KAFKA_IO_THREADS=%s
              sed -ie "s/BROKERID/2/g" server.properties
              sed -ie "s/ZOOKEEPER_SERVER_ADDR/$ZOOKEEPER_SERVER_ADDR/g" server.properties
              sed -ie "s/KAFKA_NETWORK_THREADS/$KAFKA_NETWORK_THREADS/g" server.properties
              sed -ie "s/KAFKA_IO_THREADS/$KAFKA_IO_THREADS/g" server.properties
              SED_KAFKA_LOG=$(sed 's/[\/&]/\\&/g' <<<"$KAFKA_LOG")                          
        ...
        steps: [           
          ...
          bash(
            |||            
              ...
              KAFKA_ENDPOINTS="[\"${KAFKA_SERVER_1_ADDR}:9092\",\"${KAFKA_SERVER_2_ADDR}:9092\",\"${KAFKA_SERVER_3_ADDR}:9092\"]"
              ...
              sleep 5
              for RPS in $(seq 900000 900000 900000); do
